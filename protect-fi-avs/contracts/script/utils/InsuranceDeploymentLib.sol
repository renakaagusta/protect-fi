...
library InsuranceDeploymentLib {
    using stdJson for *;
    using Strings for *;
    ...
    struct DeploymentData {
        address insuranceServiceManager;
    ...
    function deployContracts(
        ...
        result.insuranceServiceManager = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        result.stakeRegistry = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        // Deploy the implementation contracts, using the proxy contracts as inputs
        address stakeRegistryImpl =
            address(new ECDSAStakeRegistry(IDelegationManager(core.delegationManager)));
        address insuranceServiceManagerImpl = address(
            new InsuranceServiceManager(
                core.avsDirectory, result.stakeRegistry, core.rewardsCoordinator, core.delegationManager
            )
        );
        // Upgrade contracts
        bytes memory upgradeCall = abi.encodeCall(
            ECDSAStakeRegistry.initialize, (result.insuranceServiceManager, 0, quorum)
        );
        UpgradeableProxyLib.upgradeAndCall(result.stakeRegistry, stakeRegistryImpl, upgradeCall);
        UpgradeableProxyLib.upgrade(result.insuranceServiceManager, insuranceServiceManagerImpl);
    ...
    function readDeploymentJson(
        ...
        data.insuranceServiceManager = json.readAddress(".contracts.insuranceServiceManager");
    ...
    function writeDeploymentJson(
        ...
        writeDeploymentJson("deployments/insurance/", block.chainid, data);
    }
    ...
    function writeDeploymentJson(
        ...
        address proxyAdmin =
            address(UpgradeableProxyLib.getProxyAdmin(data.insuranceServiceManager));
    ...
    function _generateContractsJson(
        ...
        return string.concat(
            ...
            data.insuranceServiceManager.toHexString(),
            '","insuranceServiceManagerImpl":"',
            data.insuranceServiceManager.getImplementation().toHexString(),
            '","stakeRegistry":"',
